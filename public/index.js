...
const LANG = {
  ar: {
    ...
    wallpaperVideoTooLarge: 'حجم فيديو الخلفية يجب ألا يتجاوز 20 ميغابايت', wallpaperVideoBadType: 'صيغة الفيديو غير مدعومة، استخدم MP4 أو WebM',
  ...
  en: {
    ...
    wallpaperVideoTooLarge: 'Background video must be under 20MB', wallpaperVideoBadType: 'Unsupported video format, use MP4 or WebM',
...
// ── تخزين فيديو الخلفية محلياً (IndexedDB) — مجرد كاش على هذا الجهاز/المتصفح
// فقط، لا يُرفع لأي سيرفر إطلاقاً. localStorage غير مناسب لملفات كبيرة كالفيديو
// (محدود المساحة ومتزامن/يجمّد الصفحة)، فنستخدم IndexedDB المخصص لذلك.
const WP_VIDEO_DB = 'hostaka-wallpaper-video';
const WP_VIDEO_STORE = 'videos';
function wpVideoDbOpen(){
  return new Promise((resolve, reject) => {
    const req = indexedDB.open(WP_VIDEO_DB, 1);
    req.onupgradeneeded = () => { req.result.createObjectStore(WP_VIDEO_STORE); };
    req.onsuccess = () => resolve(req.result);
    req.onerror = () => reject(req.error);
  });
}
async function wpVideoSet(blob){
  const db = await wpVideoDbOpen();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(WP_VIDEO_STORE, 'readwrite');
    tx.objectStore(WP_VIDEO_STORE).put(blob, 'current');
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}
async function wpVideoGet(){
  try{
    const db = await wpVideoDbOpen();
    return new Promise((resolve, reject) => {
      const tx = db.transaction(WP_VIDEO_STORE, 'readonly');
      const req = tx.objectStore(WP_VIDEO_STORE).get('current');
      req.onsuccess = () => resolve(req.result || null);
      req.onerror = () => reject(req.error);
    });
  }catch(e){ return null; }
}
async function wpVideoDelete(){
  try{
    const db = await wpVideoDbOpen();
    return new Promise((resolve) => {
      const tx = db.transaction(WP_VIDEO_STORE, 'readwrite');
      tx.objectStore(WP_VIDEO_STORE).delete('current');
      tx.oncomplete = () => resolve(true);
      tx.onerror = () => resolve(false);
    });
  }catch(e){ return false; }
}
let wpVideoObjectUrl = null; // نتتبعه عشان نحرره (revoke) ونمنع تسريب الذاكرة
...
function openWallpaperModal(){
  ...
  if(wallpaperData && (wallpaperData.img || wallpaperData.type === 'video')){
    // للفيديو نعرض إطاراً مصغّراً (poster) التقطناه وقت الاختيار كمعاينة خفيفة
    box.style.backgroundImage = `url(${wallpaperData.type === 'video' ? wallpaperData.poster : wallpaperData.img})`;
...
async function onWallpaperFile(evt){
  ...
  if(file.type.startsWith('video/')){
    if(!file.type.match('video/mp4') && !file.type.match('video/webm')){ await hostakaAlert(t('wallpaperVideoBadType')); return; }
    if(file.size > 20*1024*1024){ await hostakaAlert(t('wallpaperVideoTooLarge')); return; }
    try{
      await wpVideoSet(file);
      // نلتقط إطاراً من الفيديو لاستخدامه كمعاينة صغيرة وكمصدر لتحليل الألوان
      // التلقائي (نفس آلية الصور، بدون أي تعديل عليها)
      const poster = await captureVideoFramePoster(file);
      wallpaperData = {
        type: 'video',
        poster,
      ...
      saveWallpaperState();
      applyWallpaper();
      document.getElementById('wallpaperPreviewBox').style.backgroundImage = `url(${poster})`;
      document.getElementById('wallpaperPreviewEmpty').style.display = 'none';
    }catch(e){
      await hostakaAlert(t('uploadFail'));
    }
    evt.target.value = '';
    return;
  }
  ...
  // اختيار صورة جديدة يلغي أي فيديو خلفية مخزّن سابقاً (كاش) عشان ما يضل محتفظ
  // بملف كبير غير مستخدم بالجهاز
  wpVideoDelete();
...
// يلتقط إطاراً من ملف فيديو (كـ File) ويرجعه كـ dataURL مصغّر — يُستخدم كمعاينة
// خفيفة بالمودال وكمصدر لتحليل الألوان التلقائي، بدل تخزين الفيديو كامل بأي نص
function captureVideoFramePoster(file){
  return new Promise((resolve, reject) => {
    const url = URL.createObjectURL(file);
    const v = document.createElement('video');
    ...
    v.onloadeddata = () => {
      ...
      }catch(e){ /* بعض المتصفحات ترمي هنا قبل الجاهزية الكاملة، نتابع بأي حال */ }
    };
    v.onseeked = () => {
      try{
        ...
        c.width = (v.videoWidth || maxW) * scale;
    ...
    // بعض المتصفحات ما تطلق onseeked تلقائياً إن كانت currentTime=0 أصلاً
    setTimeout(() => { if(v.readyState >= 2 && !v.currentTime) v.currentTime = 0.1; }, 400);
  });
}
...
function applyWallpaper(){
  ...
  const video = document.getElementById('wallpaperVideoBg');
  ...
  const clearVideoBg = () => {
    ...
    if(wpVideoObjectUrl){ URL.revokeObjectURL(wpVideoObjectUrl); wpVideoObjectUrl = null; }
  };

  const isVideoType = wallpaperData && wallpaperData.type === 'video';
  const hasWallpaper = wallpaperData && (wallpaperData.img || isVideoType);
  ...
  if(isVideoType){
    // خلفية الفيديو: تُقرأ من IndexedDB (كاش محلي فقط على هذا الجهاز) — لو
    // اتمسحت (تنظيف متصفح، وضع تصفح خاص...) نتجاهل بصمت ونعرض بلا خلفية
    bg.style.display = 'none';
    video.style.display = 'block';
    video.style.filter = `blur(${wallpaperData.blur ?? 6}px)`;
    wpVideoGet().then(blob => {
      if(!blob){ clearVideoBg(); html.classList.remove('has-wallpaper'); return; }
      if(wpVideoObjectUrl) URL.revokeObjectURL(wpVideoObjectUrl);
      wpVideoObjectUrl = URL.createObjectURL(blob);
      video.src = wpVideoObjectUrl;
      video.play().catch(()=>{}); // بعض المتصفحات تتطلب تفاعل المستخدم أولاً؛ لا بأس لو فشل التشغيل التلقائي
    });
  } else {
    clearVideoBg();
    bg.style.display = '';
  ...
  // تحليل الألوان التلقائي: للصورة نستخدمها مباشرة، وللفيديو نستخدم الإطار
  // المصغّر (poster) الملتقط وقت الاختيار — نفس دالة التحليل بلا أي تعديل
  const colorSourceImg = isVideoType ? wallpaperData.poster : wallpaperData.img;
  if(wallpaperData.auto !== false && colorSourceImg){
    analyzeWallpaperColors(colorSourceImg, (info) => {
      if(!info) return;
